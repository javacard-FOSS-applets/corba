...
public class LoadCFlexAccess32 implements CTListener {
	...
	private final static String[] ARGOPTIONS = { "-load", "-statics",
			"-install", "-setup", "-status", "-delete", "-auth_enc", "-mac",
			"-transport_key", "-chv0", "-ublock_chv0", "-chv1", "-ublock_chv1",
			"-package_id", "-applet_id" };
	...
	private final static int[] ARGTAGS = { ARG_LOAD, ARG_STATICS, ARG_INSTALL,
			...
			ARG_CHV0, ARG_UBLKCHV0, ARG_CHV1, ARG_UBLKCHV1, ARG_PKGID,
			ARG_APPID };
		...
		while (cmdlparser.more()) {
			...
			case ARG_DELETE:
				...
				if (cmdlparser.more()) {
					...
					System.out.println(cmderrmissingval
							+ cmdlparser.currentArgString());
					usage();
					System.exit(1);
				}
				break;
